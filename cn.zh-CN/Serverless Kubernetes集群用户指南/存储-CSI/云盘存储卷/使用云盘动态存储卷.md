...
-   已创建ASK集群。具体操作，请参见[ASK使用快速入门](/cn.zh-CN/Serverless Kubernetes集群用户指南/集群/创建Serverless Kubernetes集群.md)。
-   已通过kubectl工具连接ASK集群。具体操作，请参见[通过kubectl连接Kubernetes集群](/cn.zh-CN/Serverless Kubernetes集群用户指南/集群/管理和访问集群/通过kubectl连接Kubernetes集群.md)。

有关StorageClass的详细说明，请参见[存储类（StorageClass）](/cn.zh-CN/Serverless Kubernetes集群用户指南/存储-CSI/云盘存储卷/存储类（StorageClass）.md)。
