...
-   集群
    -   支持集群多命名空间免密拉取。
    -   集群版本不低于1.11.2。否则，请您手动升级。具体操作，请参见[升级集群](/cn.zh-CN/Kubernetes集群用户指南/集群/升级集群/升级集群.md)。
...
注册集群支持同时拉取企业版和个人版的私有镜像，只拉取企业版的私有镜像，只拉取个人版的私有镜像。根据您的使用场景，按照以下方式修改配置项**acr-configuration**中的configMap。具体操作，请参见[配置组件](/cn.zh-CN/Kubernetes集群用户指南/多云混合云/注册集群管理/通过自建Kubernetes集群的免密组件拉取容器镜像.md)。配置内容如下：
...
例如，个人版的镜像仓库同时拉取北京地域与杭州地域的镜像，配置如下。具体操作，请参见[配置组件](/cn.zh-CN/Kubernetes集群用户指南/多云混合云/注册集群管理/通过自建Kubernetes集群的免密组件拉取容器镜像.md)。
