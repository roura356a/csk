...
-   在KMS控制台已创建用户主密钥，详细介绍请参见[管理密钥](/cn.zh-CN/用户指南/管理密钥.md)。
...
4.  在**ACK托管版**页签找到**Secret落盘加密**，选中**选择KMS密钥**，在下拉框中选择KMS密钥ID。创建ACK Pro集群的其他配置请参见[创建Kubernetes Pro版集群](/cn.zh-CN/Kubernetes集群用户指南/集群管理/创建集群/创建Kubernetes Pro版集群.md)。
