...
ACK暂不支持通过API方式为已有集群开启SNAT功能。关于SNAT相关的说明，请参见[什么是NAT网关](/cn.zh-CN/产品简介/什么是NAT网关.md)。
...
1.  创建NAT网关。
    ...
    **说明：** 默认只有VPC的第一个NAT网关，才会自动创建路由条目。对于多个NAT网关，您需手动为其配置路由。具体操作，请参见[添加和删除路由表中的路由条目](/cn.zh-CN/网络连接/路由策略/使用路由表.md)。
