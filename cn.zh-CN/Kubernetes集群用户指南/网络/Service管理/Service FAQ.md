...
如果您的CCM组件版本大于或等于**v1.9.3.276-g372aa98-aliyun**的话，请[提交工单](https://selfservice.console.aliyun.com/ticket/createIndex)。
...
1.  登录到Kubernetes集群Master节点，请参见[t16645.md\#](/cn.zh-CN/Kubernetes集群用户指南/集群/连接集群/通过kubectl连接Kubernetes集群.md)。
...
5.  根据步骤3生成的KEY值和VALUE值（分别对应上图的1和2），为该SLB打上一个TAG。详情请参见[添加标签](/cn.zh-CN/传统型负载均衡CLB/CLB用户指南/实例/标签/概述.md)。
...
2.  在SLB控制台中删除Service中port对应的监听。关于如何删除SLB监听，请参见[管理监听转发规则](/cn.zh-CN/应用型负载均衡ALB/ALB监听/管理监听转发规则.md)。 |
