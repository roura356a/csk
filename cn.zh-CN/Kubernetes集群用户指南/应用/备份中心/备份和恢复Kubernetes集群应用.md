...
-   备份应用时，正在被删除的资源不会被备份。
-   如果想使用云盘快照来进行卷的备份，需要安装CSI插件并且CSI版本大于或等于1.1.0。关于如何安装CSI插件的详情，请参见[t1592079.md\#](/cn.zh-CN/Kubernetes集群用户指南/存储-CSI/安装与升级CSI组件.md)。
...
[使用Migrate Controller备份和恢复应用](/cn.zh-CN/Kubernetes集群用户指南/应用/备份中心/使用Migrate Controller备份和恢复应用.md)
