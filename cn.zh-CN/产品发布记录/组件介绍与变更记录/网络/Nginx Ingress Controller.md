...
关于Nginx Ingress Controller的使用，请参见[通过Ingress实现灰度发布和蓝绿发布](/cn.zh-CN/Kubernetes集群用户指南/网络/Ingress管理/通过Ingress实现灰度发布和蓝绿发布.md)和[通过Ingress Controller来实现应用的流量复制](/cn.zh-CN/Kubernetes集群用户指南/网络/Ingress管理/通过Ingress Controller来实现应用的流量复制.md)。
