...
当前您可以在容器服务控制台集群详情页面，获取到当前登录用户对指定集群的 kubeconfig 访问凭证，或者通过调用 [openapi](../../../../intl.zh-CN/开发指南/集群 API 列表/获取集群 kubeconfig 接口.md#) 的方式获取指定 config内容，当您的阿里云账号具有多个集群的访问权限时，可以通过如下命令合并多个集群的 kubeconfig 内容合并到一个统一的凭证文件中。
