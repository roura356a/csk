...
-   `none`：表示启用现有的默认CPU亲和性方案。
 ...
 默认值：`none`。 |
|labels|Array of [tag](/cn.zh-CN/API参考/通用数据结构.md)|Body|否| |仅为 ECS 实例添加标签
...
|taints|Array of [taint](/cn.zh-CN/API参考/通用数据结构.md)|Body|否| |污点配置。 |
...
|auto\_renew\_period|Long|Body|否|1|节点池节点自动续费周期，当选择预付费和自动续费时才生效，且为必选值。当`PeriodUnit=Month`时，取值范围：\{1, 2, 3, 6, 12\}。
 ...
 默认值：1 |
|data\_disks|Array of [data\_disk](/cn.zh-CN/API参考/通用数据结构.md)|Body|否| |节点池节点数据盘配置。 |
