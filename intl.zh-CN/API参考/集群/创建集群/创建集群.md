...
|container\_cidr|String|Body|是|172.20.0.0/16|Pod网络地址段，必须是有效的私有网段，即以下网段及其子网：10.0.0.0/8，172.16-31.0.0/12-16，192.168.0.0/16。不能与VPC及VPC内已有Kubernetes集群使用的网段重复，创建成功后不能修改。
 ...
 有关集群网络规划，请参见[VPC下 Kubernetes 的网络地址段规划](~~86500~~)。
...
|vswitch\_ids|Array of String|Body|是|vsw-2ze3ds0mdip0hdz8i\*\*\*\*|虚拟交换机ID。 |
...
|profile|String|Body|是|Default|边缘集群标识，当创建集群类型为边缘托管版时，该参数必填。
