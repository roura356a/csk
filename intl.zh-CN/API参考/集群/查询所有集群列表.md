...
|cluster\_type|String|Query|否|Kubernetes|集群类型，取值：

 -   `Kubernetes`: 专有版集群。
-   `ManagedKubernetes`：托管版集群。
-   `Ask`：Serverless集群。
-   `ExternalKubernetes`：注册集群。 |
|page\_size|Long|Query|否|10|每页显示的记录数。 |
|page\_number|Long|Query|否|3|总页数。 |
...
-   `ack.standard`：标准托管集群。 |
|maintenance\_window|maintenance\_window| |集群维护窗口，该功能只在专业托管版中生效。 |
...
```
GET /api/v1/clusters?page_number=3&name=cluster-demo&cluster_type=Kubernetes&page_size=10&page_number=3
```
