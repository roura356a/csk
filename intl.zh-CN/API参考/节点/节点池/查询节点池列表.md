...
|labels|Array of [tag](t.md#)| |ECS标签。 |
|runtime|String|docker|容器运行时名称。 |
|runtime\_version|String|19.03.5|容器运行时版本。 |
|taints|Array of [taint](t.md#)| |节点污点信息。污点和容忍度（Toleration）相互配合，可以用来避免Pod被分配到不合适的节点上。更多信息，请参见[taint-and-toleration](https://kubernetes.io/zh/docs/concepts/scheduling-eviction/taint-and-toleration/)。 |
...
|data\_disks|Array of [data\_disk](t.md#)| |节点数据盘类型、大小等配置的组合。 |
...
|system\_disk\_size|Long|120|节点系统盘大小，单位为GiB。
 ...
 取值范围：\[20,500\]。 |
|tags|Array of [tag](t.md#)| |节点标签。标签定义规则：
