...
-   `none`：表示启用现有的默认CPU亲和性方案。
 ...
 默认值：`none`。 |
|labels|Array of [tag](/intl.zh-CN/API参考/通用数据结构.md)|Body|否| |ECS标签。 |
|runtime|String|Body|否|docker|容器运行时名称。 |
|runtime\_version|String|Body|否|19.03.5|容器运行时版本。 |
|taints|Array of [taint](/intl.zh-CN/API参考/通用数据结构.md)|Body|否| |节点污点配置。 |
...
|data\_disks|Array of [data\_disk](/intl.zh-CN/API参考/通用数据结构.md)|Body|否| |节点数据盘配置，取值范围\[0,10\]。最多支持添加10块数据盘。 |
...
|tags|Array of [tag](/intl.zh-CN/API参考/通用数据结构.md)|Body|否| |节点标签。 |
