...
-   应用管理
    ...
    -   应用备份和恢复：支持对Kubernetes应用进行备份和恢复。更多信息，请参见[对Kubernetes集群应用进行备份和恢复](/intl.zh-CN/Kubernetes集群用户指南/应用/对Kubernetes集群应用进行备份和恢复.md)。
-   存储与网络
    -   [存储插件](/intl.zh-CN/Kubernetes集群用户指南/存储-Flexvolume/存储插件说明.md)：支持Flexvolume以及CSI。
    -   存储卷和存储声明：
