...
|版本号|镜像地址|变更时间|变更内容|
|---|----|----|----|
|v1.9.3.162-g104068e-aliyun|registry.cn-hangzhou.aliyuncs.com/acs/cloud-controller-manager-amd64:v1.9.3.162-g104068e-aliyun|2019年9月6日| -   支持通过annotation：service.beta.kubernetes.io/alicloud-loadbalancer-cert-id 更新证书。
-   支持通过annotation：service.beta.kubernetes.io/alicloud-loadbalancer-forward-port 实现http到https的端口转发。
...
-   支持在Terway 网络模式下，通过annotation：service.beta.kubernetes.io/backend-type："eni" 将Pod挂载到SLB后端，提升网络转发性能。
-   支持service的externalTrafficPolicy=Local模式。可以根据Node上的Pod数量为Node设置权重。
