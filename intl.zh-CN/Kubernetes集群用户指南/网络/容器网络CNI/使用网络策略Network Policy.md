...
-   如果您在创建Terway集群时，未开启网络策略功能，可以通过**配置管理**入口，在kube-system命名空间下的**eni-config**配置项中，将disable\_network\_policy设置成false，以开启网络策略功能。关于配置项的设置，请参见[修改配置项](/intl.zh-CN/Kubernetes集群用户指南/应用/配置项及保密字典/管理配置项.md)。
