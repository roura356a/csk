...
日志服务SLS（Log Service）是针对日志类数据的一站式服务。您无需开发就能快捷完成日志数据采集、消费、投递以及查询分析等功能。在Knative中结合日志服务，能有效提升对Serverless应用的运维能力。
...
1.  为Helloworld接入日志采集。
    ...
    2.  在**Project列表**区域，单击**创建Project**创建名称为helloworld的项目。详细操作，请参见[管理Logstore](/intl.zh-CN/数据采集/准备工作/管理Logstore.md)。

    3.  单击创建的helloworld项目。

    4.  在helloworld项目页面的右上角，单击**接入数据**。

    5.  在接入数据页面选择**Docker标准输出**，进入Docker标准输出配置页面。

        ![指定采集模式](https://static-aliyun-doc.oss-accelerate.aliyuncs.com/assets/img/zh-CN/9995659951/p49477.png)

    6.  在选择日志空间配置中，确认日志空间信息并单击**下一步**。

    7.  设置**创建机器组**。

        若您已经安装Logtail日志组件，请单击**使用现有机器组**。
    ...
    10. 设置**数据源设置**。
        ...
        插件配置这里我们针对helloworld-go Service，设置采集的环境变量为：`"K_SERVICE": "helloworld-go"`。并且通过processors分割日志信息，如这里`"Keys": [ "time","level", "msg" ]`。插件配置示例如下。
    ...
    12. 在查询分析配置页面，开启**全文索引**，设置查询字段索引属性。
