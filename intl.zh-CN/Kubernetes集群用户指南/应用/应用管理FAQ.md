...
1.  登录到Kubernetes集群。更多信息，请参见[通过kubectl管理Kubernetes集群](/intl.zh-CN/Kubernetes集群用户指南/集群/连接集群/通过kubectl管理Kubernetes集群.md)。
    ...
    -   自动配置私有镜像实现无密钥编排
        ...
        **说明：** 为了避免每次使用私有镜像部署时，都需要引用密钥，您可将Secret添加到Namespace的Default Service Account中。更多信息，请参见[Add ImagePullSecrets to a service account](https://kubernetes.io/docs/tasks/configure-pod-container/configure-service-account/#add-imagepullsecrets-to-a-service-account)。
        ...
        1.  执行以下命令，查看创建的拉取私有镜像的Secret。
            ...
            本例中采用手动配置的方式，修改命名空间的默认服务帐户**Default**，从而将此Secret作为imagePullSecret。
        ...
        3.  执行`vim sa.yaml`命令，删除resourceVersion，并增加拉取镜像的密钥配置项imagePullSecrets。修改后的配置如下所示：
        ...
        4.  执行以下命令将sa.yaml配置文件替换Default的服务账号配置。
