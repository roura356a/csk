...
1.  登录到Master节点，执行以下命令查询集群的GPU共享能力。
    ...
    **说明：**
    ...
    -   关于如何登录到Master节点，请参见[通过密码认证登录Linux实例](/intl.zh-CN/实例/连接实例/使用VNC连接实例/通过密码认证登录Linux实例.md)和[通过密码认证登录Windows实例](/intl.zh-CN/实例/连接实例/使用VNC连接实例/通过密码认证登录Windows实例.md)。
    -   如果您需要在本地客户端执行命令查询集群的GPU共享能力，请安装共享GPU组件和资源工具。具体操作，请参见[步骤四：安装和使用GPU资源查询工具](/intl.zh-CN/Kubernetes集群用户指南/ACK Pro集群/GPU资源调度/共享GPU专业版/安装并使用共享GPU组件和资源工具.md)。
    ```
