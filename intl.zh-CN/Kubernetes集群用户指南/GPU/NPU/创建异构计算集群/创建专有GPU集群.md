...
您可设置是否开放**使用EIP暴露API Server**。API Server提供了各类资源对象（Pod，Service等）的增删改查及Watch等HTTP Rest接口。
        ...
        -   选择不开放公网SSH登录，将无法通过SSH访问集群，也无法通过kubectl连接集群。如果您需要通过SSH访问集群实例，可以手动为ECS实例绑定EIP，并配置安全组规则，开放SSH（22）端口，具体操作参见[t16642.md\#](/intl.zh-CN/Kubernetes集群用户指南/集群/连接集群/通过SSH访问Kubernetes集群.md)。 |
...
**说明：**
        ...
        |**服务帐户令牌卷投影**|开启**服务账户令牌卷投影**功能以降低在Pod中使用ServiceAccount遇到的安全性问题。详情请参见[部署服务账户令牌卷投影](/intl.zh-CN/Kubernetes集群用户指南/安全/基础设施安全/部署服务账户令牌卷投影.md)。 |
...
选中**使用日志服务**后，您还可以选择是否在日志服务控制台中**创建Ingress Dashboard**或者**安装node-problem-detector并创建事件中心**。 |
    |**工作流引擎**|设置是否使用AGS。 **说明：** 目前只有白名单用户才能使用该功能。
