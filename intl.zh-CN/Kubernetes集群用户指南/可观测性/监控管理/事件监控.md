...
1.  对被监控集群安装NPD并激活SLS功能。具体安装步骤，请参见[场景3](#section_rya_7i0_9e3)。
...
3.  在应用目录 - ack-node-problem-detector页面的**参数**页签，可以看到node-problem-detector的默认配置。
    ...
    |插件名|功能|说明|
    |---|--|--|
    |fd\_check|检查集群节点系统的已打开的文件描述符是否超过最大上限的80%。|80%为默认值，可通过参数修改。默认开启项。|
    ...
    |docker\_offline\_check|检测集群节点上的docker daemon服务是否正常。|无|
...
1.  开通事件总线EventBridge。具体操作，请参见[开通事件总线EventBridge]()。
