...
-   集群
    ...
    -   支持的集群版本：
        -   专有版Kubernetes集群：高于或等于1.11.2的版本默认支持免密拉取镜像。低于1.11.2版本请您手动升级，请参见[升级集群](/intl.zh-CN/Kubernetes集群用户指南/集群管理/升级集群/升级集群.md)。
...
2.  配置组件。
    ...
    5.  在**配置项**页面，单击目标**配置项名称**，单击**编辑**，修改**配置项**名称。
        ...
        如果您没有该配置项，请参见[创建配置项](/intl.zh-CN/Kubernetes集群用户指南/配置项及密钥管理/创建配置项.md)。更新配置项，请参见[修改配置项](/intl.zh-CN/Kubernetes集群用户指南/配置项及密钥管理/修改配置项.md)。
...
**使用自定义RAM角色的访问密钥进行跨账号私有镜像拉取**

1.  创建一个受信实体为**阿里云账号**类型的RAM角色并为该角色添加cr镜像拉取权限。

    具体操作步骤，请参见**使用角色扮演进行跨账号拉取**[步骤1](#step_5b4_2v8_wr8)。

2.  配置**kube-system**命名空间中的配置项acr-configuration，并填入创建的RAM角色的访问密钥AccessKey ID和AccessKey Secret。

    此种方式可以使免密组件直接使用创建的RAM角色拉取私有镜像。查看访问密钥的具体步骤，请参见[查看访问密钥基本信息](/intl.zh-CN/安全设置/访问密钥/查看访问密钥基本信息.md)。
    ...
    data:   
        ...
        acr-registry-info: |
        - instanceId: ""
          customAccessKey: "xxxxx" // 此处填写刚创建的RAM角色的AccessKey ID。customAccessKeySecret: "xxxxxx" // 此处填写刚创建的RAM角色的AccessKey Secret。
    ```
