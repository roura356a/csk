...
如果您没有部署CSI-Plugin，请参见[安装CSI插件](/intl.zh-CN/Kubernetes集群用户指南/存储-CSI/安装CSI插件.md)。
...
1.  RAM Policy设置和授予。
    ...
    -   创建子账号授权以上RAM Policy并生成AccessKey，配置到StatefulSet csi-provisioner中csi-nasprovisioner的env变量中。请参见[容器服务默认角色](/intl.zh-CN/Kubernetes集群用户指南/授权/容器服务默认角色.md)。
