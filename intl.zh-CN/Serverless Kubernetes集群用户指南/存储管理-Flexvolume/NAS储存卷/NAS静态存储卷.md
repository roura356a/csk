...
1.  创建PV。
    ...
    -   通过控制台界面创建云盘数据卷。
        ...
        6.  在创建存储卷对话框中，配置存储卷的相关参数。
            ...
            |**存储驱动**|本示例选择Flexvolume。有关存储插件详细信息，请参见[Flexvolume和CSI存储插件的区别](/intl.zh-CN/Kubernetes集群用户指南/存储管理-Flexvolume/存储插件说明.md)。|
            |**总量**|所创建存储卷的容量。注意不能超过磁盘容量。|
            |**访问模式**|默认为ReadWriteMany。|
            |**挂载点域名**|集群在NAS文件系统中挂载点的挂载地址。|
...
[NAS存储卷使用说明](/intl.zh-CN/Kubernetes集群用户指南/存储管理-Flexvolume/NAS储存卷/NAS存储卷使用说明.md)
