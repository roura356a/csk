---
keyword: [Windows node, Logtail, collect application log]
---

# Use Logtail to collect application log from Windows nodes

You can use Logtail to collect application log from Windows nodes. This topic describes how to use Logtail to collect application log from Windows nodes.
...
**Note:** Logtail v1.1.x is reliant on the privileged proxy processes on Windows nodes. Logtail V1.1.x can be deployed only on Windows nodes with the `window.alibabacloud.com/deployment-topology=2.0` label.
