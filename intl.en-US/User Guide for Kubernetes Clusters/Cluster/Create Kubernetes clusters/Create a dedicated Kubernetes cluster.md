...
A dedicated Kubernetes cluster must contain at least three master nodes. This ensures high availability and allows fine-grained management of the cluster infrastructure. You must manually create, maintain, and upgrade dedicated Kubernetes clusters. This topic describes how to create a dedicated Kubernetes cluster in the Container Service for Kubernetes \(ACK\) console.
...
5.  Click **Next:Master Configurations** to configure master nodes.
    ...
    |**System Disk**|By default, system disks are mounted to master nodes. Standard SSDs, enhanced SSDs, and ultra disks are supported. **Note:**
...
7.  Click **Next:Component Configurations** to configure components.
    ...
    |**Monitoring Agents**|Select whether to install the CloudMonitor agent. By default, **Install CloudMonitor Agent on ECS Instance** is selected. |
