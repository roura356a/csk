...
A cluster of Container Service for Kubernetes \(ACK\) that runs sandboxed containers is created. For more information, see [Create a managed ACK cluster that supports sandboxed containers](/intl.en-US/User Guide for Kubernetes Clusters/Sandboxed-Container management/Create a managed ACK cluster that supports sandboxed containers.md).
