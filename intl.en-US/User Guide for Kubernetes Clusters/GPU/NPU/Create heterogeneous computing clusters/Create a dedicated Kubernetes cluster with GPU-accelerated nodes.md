...
1.  In the Select Cluster Template dialog box, find **Dedicated Cluster for Heterogeneous Computing** in the **Other Clusters** section and click **Create**.

2.  On the **Dedicated Kubernetes** tab, configure the cluster.
...
**Note:** ACK clusters support only VPCs. You can select a VPC from the drop-down list. If no VPC is available, click **Create VPC** to create one. For more information, see [t2435.md\#](/intl.en-US/VPCs and vSwitchs/Work with VPCs.md). |
        ...
        |**Network Plug-in**|Select and configure the network plug-in. Flannel and Terway are available. For more information, see [t2436.md\#section\_ts9\_t3s\_8vw](/intl.en-US/VPCs and vSwitchs/Work with vSwitches.mdsection_ts9_t3s_8vw).         -   Flannel: an open source Container Network Interface \(CNI\) plug-in, which is simple and stable. Flannel provides a few simple features and does not support standard Kubernetes network policies.
...
**Note:** This feature is available to only users in the whitelist. To apply to be added to the whitelist, [submit a ticket](https://selfservice.console.aliyun.com/ticket/scene/ecs/%E4%BA%91%E6%9C%8D%E5%8A%A1%E5%99%A8%20ECS/detail). |
        ...
        |**Security Group**|You can select **Create Basic Security Group**, **Create Advanced Security Group**, or **Select Existing Security Group**. For more information, see [t9569.md\#](/intl.en-US/Security/Security groups/Overview.md). |
...
3.  Click **Next:Master Configurations** to configure master nodes.
    ...
    |**Instance Type**|Select an instance type for the master nodes. For more information, see [t9548.md\#](/intl.en-US/Instance/Instance families.md).**Note:** If no instance type is available, you can change vSwitches on the **Cluster Configurations** wizard page. |
...
4.  Click **Next:Worker Configurations** to configure worker nodes.
    ...
    2.  Configure advanced settings.
        ...
        |**User Data**|For more information, see [t9660.md\#](/intl.en-US/Instance/Manage instances/Manage instance user data/Overview of ECS instance user data.md).|
...
5.  Click **Next:Component Configurations** to configure components.
    ...
    |Parameter|Description|
    |---------|-----------|
    |**Ingress**|Specify whether to install Ingress controllers. By default, **Install Ingress Controllers** is selected. For more information, see [t16679.md\#](/intl.en-US/User Guide for Kubernetes Clusters/Network/Ingress management/Configure an Ingress.md).|
...
After you select **Enable Log Service**, you can specify whether to select **Create Ingress Dashboard** and **Install node-problem-detector and Create Event Center**. |
    ...
    -   If you clear this check box, you must manually install the AGS workflow plug-in. For more information, see [t422672.md\#](/intl.en-US/User Guide for Genomics Service/AGS workflow/Introduction to AGS CLI.md). |
