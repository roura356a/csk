...
5.  Click the **Cluster Events Management** tab. In the left-side navigation pane of the **K8s Event Center** page, find the cluster that you want to manage. Then, click the ![Show](https://static-aliyun-doc.oss-accelerate.aliyuncs.com/assets/img/en-US/5301471161/p203800.png) icon to the left of the cluster name to view event details that are provided by the Kubernetes event center.
...
3.  On the App Catalog - ack-node-problem-detector page, click the **Parameters** tab to view the default configurations of node-problem-detector.
    ...
    |-------|--------|-----------|
    |fd\_check|Checks whether the percentage of opened file descriptors on each cluster node exceeds 80%.|The default threshold is 80%. The threshold is adjustable. The plug-in is enabled by default.|
    ...
    |docker\_offline\_check|Checks whether the docker daemon functions as expected on cluster nodes.|N/A|
...
1.  Activate EventBridge. For more information, see [Activate EventBridge]().
